# -*- coding: utf-8 -*-
"""Mock 测试：配置存储、DPAPI 加密、登录目标解析。

全部在临时目录里跑，不碰真实的 config.json。
...
def test_entry_completeness():
    entry = ConnectionEntry(connection="DEV-1", client="120", user="DEMO_USER", password="x")
    assert entry.is_complete is True
    assert entry.missing_fields() == []

    entry = ConnectionEntry(connection="DEV-1")
    assert entry.is_complete is False
    assert entry.missing_fields() == ["client", "用户名", "密码"]


def test_entry_display_name():
    assert ConnectionEntry(connection="DEV-1", client="120").display_name == "DEV-1 / 120"
    assert ConnectionEntry(connection="DEV-1", label="生产").display_name == "生产"


def test_entry_repr_hides_password():
    entry = ConnectionEntry(connection="DEV-1", client="120", user="u", password="topsecret")
...
def test_save_and_load_roundtrip():
    with make_temp_dir() as folder:
        ...
        config = AppConfig(
            ...
            entries=[
                ConnectionEntry(
                    connection="DEV-1", client="120", user="DEMO_USER",
...
def sample_entries():
    return [
        ConnectionEntry(connection="DEV-1", client="120", user="u1", password="p1", label="生产120"),
        ConnectionEntry(connection="DEV-1", client="110", user="u1", password="p1", label="生产110"),
        ConnectionEntry(connection="PRD-1", client="", user="u3", password="p3", label="测试3P"),
    ]
...
def test_resolve_by_exact_client():
    ...
    assert target.connection == "DEV-1"


def test_resolve_by_connection_name():
    """client 栏是空的条目，按连接名也能选中，并且 client 沿用用户输入。"""
    settings = build_settings(sample_entries(), client_rules=(("1", "PRD-1"),))
    target = resolve_target(settings, "PRD-1")
    assert target.connection == "PRD-1"
...
def test_resolve_by_legacy_prefix_rule():
    """旧快捷方式 OpenSAPGUI.exe 100 -> 走 PRD-1，client 用输入的完整值。"""
    settings = build_settings(sample_entries(), client_rules=(("1", "PRD-1"),))
    target = resolve_target(settings, "100")
    assert target.connection == "PRD-1"
    assert target.client == "100"
...
def test_entry_repr_and_from_json_tolerate_int_client():
    ...
    entry = ConnectionEntry.from_json({
        "connection": "DEV-1", "client": 120, "user": "u", "password": "p",
    })
    assert entry.client == "120"
...
def test_foreign_password_keeps_other_fields():
    ...
    with make_temp_dir() as folder:
        ...
        write_foreign_config(path, [{
            "id": "aaa", "connection": "DEV-1", "client": "120", "user": "DEMO_USER",
        ...
        assert (entry.connection, entry.client, entry.user) == ("DEV-1", "120", "DEMO_USER")
...
def test_foreign_password_counts_and_keeps_usable_entries():
    ...
    with make_temp_dir() as folder:
        path = pathlib.Path(folder) / "config.json"
        write_foreign_config(path, [
            {"id": "aaa", "connection": "DEV-1", "client": "120", "user": "u1",
             "password": foreign_password()},
            {"id": "bbb", "connection": "PRD-1", "client": "300", "user": "u2",
        ...
        good = config.find_entry_by_connection("PRD-1")
...
def test_foreign_password_copy_keeps_flag():
    ...
    entry = ConnectionEntry(connection="DEV-1", client="120", user="u",
...
def test_pinned_roundtrip():
    ...
    with make_temp_dir() as folder:
        ...
        config = AppConfig(entries=[
            ConnectionEntry(connection="DEV-1", client="120", user="u",
                            password=FAKE_PASSWORD, pinned=True),
            ConnectionEntry(connection="PRD-1", client="300", user="u",
...
def test_sort_entries_same_name_uses_client():
    ...
    make = lambda client: ConnectionEntry(  # noqa: E731
        connection="DEV-1", client=client, password="x"
    )
    ordered = sort_entries([make("300"), make("110"), make("200")])
    assert [e.client for e in ordered] == ["110", "200", "300"]
...
def test_options_roundtrip_keeps_dropdown_sources():
    ...
    assert again.client_candidate_fallback() == ["100", "200", "300"]
...
def test_config_file_stores_dropdown_sources():
    with make_temp_dir() as folder:
        ...
        config = AppConfig(
            ...
            entries=[ConnectionEntry(connection="DEV-1", client="120",
...
def test_entry_tcode_field_persists():
    with make_temp_dir() as folder:
        ...
        config = AppConfig(entries=[ConnectionEntry(
            connection="DEV-1", client="110", user="u",
...
def test_entry_tcode_defaults_empty():
    ...
    entry = ConnectionEntry.from_json({"connection": "DEV-1", "client": "120"})
    assert entry.tcode == ""


def test_resolve_target_carries_tcode():
    config = AppConfig(entries=[ConnectionEntry(
        connection="DEV-1", client="110", user="u", password=FAKE_PASSWORD, tcode="SE80",
    )])
...
def test_match_environment_by_suffix_and_keyword():
    ...
    assert match_environment(rules, "APP-D") == "开发"
    assert match_environment(rules, "APP-Q") == "测试"
    assert match_environment(rules, "APP-P") == "生产"
    assert match_environment(rules, "app-q") == "测试", "大小写不敏感"
    assert match_environment(rules, "ZZ-9X") == "", "不命中返回空"
    assert match_environment([], "APP-D") == ""


def test_match_environment_by_client_and_priority():
    rules = [["client:100", "生产"], ["*P", "生产"], ["120", "特殊120"]]
    assert match_environment(rules, "APP-D", "100") == "生产"
    assert match_environment(rules, "APP-D", "120") == "特殊120", "包含匹配也能命中"
    assert match_environment(rules, "APP-P", "610") == "生产"
    ordered = [["*P", "生产"], ["client:610", "测试"]]
    assert match_environment(ordered, "APP-P", "610") == "生产", "*P 先命中就用它"
    assert match_environment(ordered, "APP-Q", "610") == "测试"


def test_options_env_rules_roundtrip():
    options = AppOptions(env_rules=[["*D", "开发"], ["client:100", "生产"]])
    again = AppOptions.from_json(json.loads(json.dumps(options.to_json())))
    assert again.env_rules == [["*D", "开发"], ["client:100", "生产"]]
